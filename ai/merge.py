...
# Function to determine season from match date
...
# Function to find matching team name using regex
...
# Standardize team names dynamically
for league in matches_df['League'].unique():
    for season in matches_df['Season'].unique():
        # Get teams from matches and stats for this league and season
        ...
        # Create a mapping for this league and season
        temp_mapping = {}
        unmapped = []
...
def standardize_result(row):
    ...
    if result == "Draw":
        return "Draw"
    # Special case for Borussia Mönchengladbach
    ...
    if away_team in result:
        return "Away Win"
    return result  # Fallback for debugging


training_df['Result'] = training_df.apply(standardize_result, axis=1)

# Debug: Check for unmapped results
